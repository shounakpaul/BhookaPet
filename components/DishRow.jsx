...
export default function DishRow({ id, name, description, price, image }) {
  ...
  const selectBasketItemsWithId = createSelector(
    [selectBasketItems, selectId],
    ...
    <>
      ...
      >
        <View className="flex-row">
          <View className="flex-1 pr-2 font-semibold">
      ...
      {isPressed && (
        <View className="bg-white px-4">
          <View className="flex-row items-center space-x-2 pb-3 justify-center">
            <TouchableOpacity
              disabled={!items.length}
              onPress={removeItemFromBasket}
            >
              <MinusCircleIcon
                color={items.length > 0 ? "#42adff" : "gray"}
                size={40}
              />
            </TouchableOpacity>
            <Text className="">{items?.length}</Text>
            <TouchableOpacity onPress={addItemToBasket}>
              <PlusCircleIcon color="#42adff" size={40} />
            </TouchableOpacity>
          </View>
        </View>
      )}
